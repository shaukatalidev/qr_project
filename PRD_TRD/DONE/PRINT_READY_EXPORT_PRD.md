...
**Rev (2026-09-13, sheet UI shipped):** (1) **N-up sheets are Pro + Agency, not Agency.** The gate is `bulk_generation`, and `0009` gives that flag to Pro as well, so Pro could always call the endpoint; the owner chose to keep it open rather than add an Agency-only flag, and the 403 copy now names both tiers. (2) The endpoint shipped in July with **no UI** — `usePrintSheet` had no caller. Entry points now: **Print sheet** in the QR list's selection bar and on the bulk results page (dynamic batches only; static QRs have no short code and are skipped). (3) As built, the sheet takes page + columns × rows, not the Avery presets in §B; the dialog offers A4/Letter/A5 with six grid presets and quotes the printed code size, warning under 20 mm. Labels are Latin-1, so non-Latin QR names print as "?".
